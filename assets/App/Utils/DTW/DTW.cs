...
//	private void _init(float[] xsig, float[] ysig)
	private void _init(mDistanceMeasurable[] xsig, mDistanceMeasurable[] ysig, bool upper)
	{
...
//				_costMatrix[y][x] = Math.Abs(xsig[x]-ysig[y]);
				_costMatrix[y][x] = ysig[y].getDistance(xsig[x], upper);
...
//	public List<mIndex> warpDTW(float[] p1, float[] p2)
	public List<mIndex> warpDTW(mDistanceMeasurable[] p1, mDistanceMeasurable[] p2, bool p)
	{
		_init(p1, p2, p);
...
//	public List<mIndex> warpSubsequenceDTW(float[] seq, float[] subseq)
	public List<mIndex> warpSubsequenceDTW(mDistanceMeasurable[] seq, mDistanceMeasurable[] subseq, bool upper)
	{
		_init(seq, subseq,upper);
...
{
	float getDistance(mDistanceMeasurable other,bool upper);
}